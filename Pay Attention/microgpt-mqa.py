...
microgpt in PyTorch — GPU training on WikiText-103.
  RMSNorm, causal multi-query attention (MQA), MLP with ReLU, Adam.
...
class CausalAttention(nn.Module):
    def __init__(self, embd_dims, n_head, block_size):
        ...
        self.wk = nn.Linear(embd_dims, self.head_dim, bias=False)      # MQA: single KV head
    ...
    def forward(self, x):
        ...
        k = self.wk(x).view(B, T, 1, hd).transpose(1, 2)           # (B, 1, T, hd)
        v = self.wv(x).view(B, T, 1, hd).transpose(1, 2)           # (B, 1, T, hd)
